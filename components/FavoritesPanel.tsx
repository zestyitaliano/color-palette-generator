    ...
    <>
      ...
      <aside className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
            ...
            <button onClick={onClose} className="p-2 text-gray-500 hover:bg-gray-100 transition-colors" aria-label="Close favorites panel">
                <XIcon />
            </button>
        </div>
                ...
                <ul className="space-y-3">
                    {palettes.map((p, i) => (
                        <li key={i} className="group flex items-center gap-2 p-2 bg-gray-50 hover:bg-gray-100 border border-gray-100 hover:border-gray-200 transition-all">
                            ...
                            <button onClick={() => onDelete(p)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all" aria-label="Delete palette">
                                <TrashIcon />
                            </button>
                        </li>
                    ))}
